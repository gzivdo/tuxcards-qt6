...
private:
  ...
  static void parseElementAttributes( QDomElement& elem, CTreeInformationElement& infoElem );

  static QDomElement* parseCollection(CInformationCollection* collection, QDomNode node);
  static QString mpathOfLastActiveElement;

  static bool mbCurrentCollectionIsEncrypted;  // used when reading from file only
};
